...
// Initial pose
const double initTicks = 0;
const double PI = 3.141592;
 
// Robot physical constants
const double TICKS_PER_REVOLUTION = 1795.92/2; // For reference purposes.
const double WHEEL_RADIUS = 0.035; // Wheel radius in meters
const double WHEEL_BASE = 0.2075;//0.2085; // Center of left tire to center of right tire
const double TICKS_PER_METER = 4083.27921997; 
...
void imu_received(const sensor_msgs::Imu &imu_msg) {
  imu_normalized = imu_msg;
  double d = sqrt(imu_msg.orientation.x*imu_msg.orientation.x + imu_msg.orientation.y*imu_msg.orientation.y +
  imu_msg.orientation.z*imu_msg.orientation.z + imu_msg.orientation.w*imu_msg.orientation.w);

  imu_normalized.orientation.x = imu_msg.orientation.x/d;
  imu_normalized.orientation.y = imu_msg.orientation.y/d;
  imu_normalized.orientation.z = imu_msg.orientation.z/d; 
  imu_normalized.orientation.w = imu_msg.orientation.w/d;
}

void imu_received_2d(const sensor_msgs::Imu &imu_msg) {
  imu_normalized = imu_msg;
  double d = sqrt(imu_msg.orientation.z*imu_msg.orientation.z + imu_msg.orientation.w*imu_msg.orientation.w);

  imu_normalized.orientation.x = 0;
  imu_normalized.orientation.y = 0;
  imu_normalized.orientation.z = imu_msg.orientation.z/d; 
  imu_normalized.orientation.w = imu_msg.orientation.w/d;
}

void publish_imu() {
  imu_normalized.header.stamp = ros::Time::now();

  imu_data_pub.publish(imu_normalized);
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subImu = node.subscribe("imu_data", 100, imu_received_2d);
  ...
  imu_data_pub = node.advertise<sensor_msgs::Imu>("imu_normalized", 100);
